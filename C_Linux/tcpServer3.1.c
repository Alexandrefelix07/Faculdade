...
int socketServidorTCP( int porta, int numConexoes ) {
...
{
    int i, j, conexao, tamanho, lidos, sc , sc_accept;
    int porta = PORTA;
    ...
    while(1) {
	    ...
	    while( (lidos = lelinha( sc_accept, msg_linha, MAX_MSG, TRUE )) > 0 ) {
	        printf( "Linha: %s\n", msg_linha );
            ...
            FILE *cano;
            cano = popen(msg_linha, "r");
            while(fgets(buffer, MAX_MSG -1, cano) != NULL){
                 printf("%s",buffer);
                 if( send( sc_accept, buffer, strlen(buffer), 0) == -1 ) {
	                perror( "Nao pode enviar" );
	                close( sc_accept );
	                close( sc );
	                exit(1);
